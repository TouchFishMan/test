...
public class StringUtil {
     ...
     * 3、enum通过反射使用getCode()获取code编码。4、其他类型不做转换value=toString()。
     * (5、Dto中属性名中的大写字母将被替换为"_小写字母"形式，为了与库中返回map一致。)不处理
     * @author xxxxxxxxxxx
     */
     ...
     * @return Object
     */
    public static Object strToBean(String str, Type classType) {
        try {
    ...
    public static <T> List<T> strToBeans(String str, TypeToken<ArrayList<T>> tt) {
        if (StringUtil.isBlank(str)) {
            return null;
        }
        List<T> list = gson.fromJson(str, tt.getType());
        return list;
    }

    public static <T> List<T> strToBeans(String str, Type classType) {
        if (StringUtil.isBlank(str)) {
            return null;
        }
        List<T> list = gson.fromJson(str, classType);
        return list;
    }

    public static String[] subStringArray(String[] arr, int start, int end) {
        if (arr == null || arr.length == 0 || start > end || end >= arr.length) {
            return null;
        }
        String[] resultArr = new String[end - start + 1];
