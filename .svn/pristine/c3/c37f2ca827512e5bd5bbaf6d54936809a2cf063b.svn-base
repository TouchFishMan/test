...
public class ProtocalFactory {
     ...
     *
     * key: address2+prm  (prm:1-启动站, 0-从动站)-------------以主站为基准
     */
    public static Map<String, IProtocal> address2ProtocalCache = new HashMap<>();
     ...
     * @return
     */
    public static IProtocal getProtocal(String address2, Boolean isStartStation) {
        try {
            IProtocal protocal;
            if (address2 == null) {
                ...
                String key = address2 + prm;
                protocal = address2ProtocalCache.get(key);
                if (protocal == null) {
                    protocal = ProtocalManagerFactory.getProtocalManager("rgm_376_1");
                    ProtocalImpl impl = (ProtocalImpl) protocal;
                    impl.setStartStation(isStartStation);
                    address2ProtocalCache.put(key, protocal);
                }
            }
            return protocal;
        } catch (Exception e) {
